...
from modules.deduplication_module import ImprovedDeduplicationModule
...
class ImprovedModernApp:
    ...
    def setup_function_manager(self):
        ...
        # 注册AVIF转换模块
        avif_converter_module = AVIFConverterModule()
        manager.register_module(avif_converter_module)
        
        # 如果modules目录存在，也加载其中的模块
        modules_dir = 'modules'
        if os.path.exists(modules_dir):
            for filename in os.listdir(modules_dir):
                if filename.endswith('_module.py') and filename not in ['deduplication_module.py', 'avif_converter_module.py']:
